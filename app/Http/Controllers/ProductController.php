...
class ProductController extends Controller
{
    public function addStockProduct(Request $request)
    {
        TrackList::query()->create([
            ...
            'to_china' => date(now()),
            'status' => 'Получено на складе Киргизии',
            'reg_china' => 1,
            'created_at' => date(now()),
        ]);
        $count = TrackList::query()->whereDate('created_at', Carbon::today())->count();
        return redirect()->back()->with(['message' => 'Трек код успешно добавлен', 'count' => $count]);
    }
    ...
    public function almatyIn(Request $request)
    {
            $wordsFromFile[] = [
                'track_code' => $request["track_code"],
                'to_almaty' => date(now()),
                'status' => 'Получено на складе Шымкент',
                'reg_almaty' => 1,
                'updated_at' => date(now()),
            ];
    ...
    public function toShimkent(Request $request)
    {
            $wordsFromFile[] = [
                'track_code' => $request["track_code"],
                'to_shimkent' => date(now()),
                'status' => "Отправлено в Казахстан -Шымкент",
                'reg_client' => 1,
                'updated_at' => date(now()),
            ];
    ...
    public function getInfoProduct(Request $request)
    {
        ...
        $track_code_statuses =  TrackList::query()->select('to_china', 'to_almaty', 'to_client','to_shimkent')->where('track_code', $request['track_code'])->first();
        if ($track_code){
